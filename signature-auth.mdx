...
canonical = 'POST' + api_path + '' + body_hash
...
func main() {
	...
	body := `{"name":"John Doe","email":"john@example.com","phone":"9876543210","phone_country_code":"+91"}`
	bodyHash := sha256.Sum256([]byte(body))
	canonical := "POST" + apiPath + "" + hex.EncodeToString(bodyHash[:])
...
$bodyHash = hash('sha256', $body);
$canonical = 'POST' . $apiPath . '' . $bodyHash;
$signature = hash_hmac('sha256', $canonical, $secretKey);
...
public class PaywintAPI {
    public static void main(String[] args) throws Exception {
        ...
        String canonical = "POST" + apiPath + "" + bodyHash.toString();
