...
- Verify the account via micro-deposits  
...
- Making ACH payments  
- Funding wallet through the user’s bank  
...
<CardGroup cols={3}>
  ...
  <Card title="Add Bank Account" icon="plus" href="/api-reference/bank-accounts/add-bank-account" arrow="true">
    Create a new bank account and link it to a user.
  </Card>

  <Card title="Release Micro-Deposit" icon="send" href="/api-reference/bank-accounts/release-micro-deposit" arrow="true">
...
<CardGroup cols={3}>
  ...
  <Card title="Get Bank Account" icon="bank" href="/api-reference/bank-accounts/get-bank-account" arrow="true">
    Retrieve details of a specific bank account.
  </Card>

  <Card title="List Bank Accounts" icon="list" href="/api-reference/bank-accounts/list-accounts" arrow="true">
    Get all bank accounts linked to a user.
  </Card>

  <Card title="Delete Bank Account" icon="trash" href="/api-reference/bank-accounts/delete-bank" arrow="true">
...
<Steps>
  ...
  <Step title="1. Add the bank account">
    Call <code>[Add Bank Account](/api-reference/bank-accounts/add-bank-account)</code> 
...
<Steps>
  ...
  <Step title="Retrieve account details">
    Use <code>[Get Bank Account](/api-reference/bank-accounts/get-bank-account)</code>
    to access account type, last 4 digits, routing number, verification status, and metadata.
  </Step>

  <Step title="List all accounts">
    Use <code>[List Bank Accounts](/api-reference/bank-accounts/list-bank-accounts)</code>
    to fetch all accounts linked to a user, ordered by most recently added.
  </Step>

  <Step title="Set a default bank account">
    Use <code>[Set Default](/api-reference/bank-accounts/set-default-bank-account)</code> to switch the default account.
  </Step>

  <Step title="Delete an account">
    Use <code>[Delete Bank Account](/api-reference/bank-accounts/delete-bank-account)</code> 
