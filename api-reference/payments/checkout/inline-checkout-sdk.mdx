...
Use this SDK when you want the payer to complete OTP verification, wallet payment, card payment, 3DS, and any required card image upload without leaving your application.

## Load the SDK

Add a mount element, register the SDK ready callback, then load `checkout.js` from the Paywint SDK host for your environment.
...
<script>
  window.$paywint = window.$paywint || {};
  $paywint.checkout = $paywint.checkout || { ready: function () {} };
</script>

<script>
  $paywint.checkout.ready = () => {
    const pwCheckout = new window.PaywintCheckout();
...
The `style` config accepts either an object or a CSS declaration string. Object keys can be provided with or without the leading `--`.
